...
transformed data{
  ...
  array[n_s] vector[n_t] cue_out_avg = rep_array(rep_vector(0,n_t),n_s); // Average shown outcome for current cue; 0 if no prior shown outcomes
  ...
  for (s in 1:n_s) {
    ...
    for (t in 1:n_t) {
      ...
      // Update cue-specific shown-outcome state.
      if (rew_hid[s,t] == 0) {
        out_sum[f] += rew[s,t];
        out_count[f] += 1;
      }
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] w_pco = w_pco_mu + w_pco_sigma*w_pco_z; // Average shown cue-outcome effect
