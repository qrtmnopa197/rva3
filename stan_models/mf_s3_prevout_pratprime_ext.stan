...
transformed data{
  ...
  array[n_s] vector[n_t] cue_out_prev = rep_array(rep_vector(0,n_t),n_s); // Previous shown outcome for current cue
  array[n_s,n_t] int cue_out_prev_avail = rep_array(0,n_s,n_t); // 1 if previous shown outcome for cue exists
  ...
  for (s in 1:n_s) {
    array[n_f] real last_out = rep_array(0.0,n_f); // Last shown outcome for each cue in block
    array[n_f] int has_last_out = rep_array(0,n_f); // 1 if cue has prior shown outcome in block
    ...
    for (t in 1:n_t) {
      ...
      // 3) previous shown outcome for this cue exists in current block.
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] w_pco = w_pco_mu + w_pco_sigma*w_pco_z; // Previous shown cue-outcome effect
