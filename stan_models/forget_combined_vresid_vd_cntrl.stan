...
// Further, this model controls for the effect of current V_d on probability ratings
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] w_vd = w_vd_mu + w_vd_sigma*w_vd_z;
    ...
    for (s in 1:n_s) {
      ...
      for (t in 1:n_t){
        ...
        if(val_rat_num[s,t] != 0){
          ...
          val_pred[val_rat_num[s,t]] = inv_logit(w_0[s] + w_bl[s]*bl_cent[s,t] + w_tr[s]*tr_cent[s,t] + w_vd[s]*V[s,t+1,frac[s,t]]
...
model {
  ...
  w_vd_mu ~ normal(0,0.03);
  w_vd_sigma ~ normal(0,0.05);
  w_vd_z ~ std_normal();
