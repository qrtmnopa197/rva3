// Preregistered model for RVA study 3, but with forget_cns and forget_rns combined into a single parameter
// Additionally, this allows for effects of probability rating residuals on valence
...
// Furthermore, this model requires that the effects of V_d and V_nd are the same. It's the same as 
// forget_combined_vresid, except with this cahnge.
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    array[n_s] matrix[n_t,5] x_preds = rep_array(rep_matrix(0,n_t,5),n_s); // 1. reward when shown
                                                                           // 2. V
                                                                           // 3. rew_hid
                                                                           // 4. V_resid when reward shown
                                                                           // 5. V_resid when reward not shown
    x_preds[,,3] = rew_hid;
    ...
    row_vector[5] sum_preds; // Weighted sum of past predictors
    ...
    for (s in 1:n_s) {
      ...
      for (t in 1:n_t){
        ...
        if(rew_hid[s,t] == 0){
          ...
          x_preds[s,t,2] = V[s,t,frac[s,t]]; // Assign V to the vector of Vs 
          x_preds[s,t,4] = V_resid[s,t,frac[s,t]]; // Assign V_resid to the vector of V_resids when the reward is shown
        } else if (rew_hid[s,t] == 1){
          // If the reward was hidden, don't update V
          x_preds[s,t,2] = V[s,t,frac[s,t]]; // Assign V to the vector of Vs
          x_preds[s,t,5] = V_resid[s,t,frac[s,t]]; // Assign V_resid to the vector of V_resids when the reward is not shown
        }
...
model {
  ...
  wx_mu[{1, 2, 4, 5}] ~ normal(0,0.03);
  wx_sigma[{1, 2, 4, 5}] ~ normal(0,0.05);
  wx_z[,1] ~ std_normal();
  wx_z[,2] ~ std_normal();
  wx_z[,4] ~ std_normal();
  wx_z[,5] ~ std_normal();
