// Preregistered model for RVA study 3, but without the forget_rns parameter and using z-scored valence ratings.
...
data {
  ...
  vector[n_vrat] val_rat_z; // The valence rating on each trial
    ...
    for (s in 1:n_s) {
      ...
      for (t in 1:n_t){
        ...
        if(rew_hid[s,t] == 0){
          ...
          // If the reward was not shown...
          V[s,t+1,frac[s,t]] = V[s,t,frac[s,t]]; // No forgetting for the shown cue
        ...
        if(val_rat_num[s,t] != 0){
          ...
          val_pred[val_rat_num[s,t]] = w_0[s] + w_bl[s]*bl_cent[s,t] + w_tr[s]*tr_cent[s,t] + dot_product(sum_preds,wx[s]);
        }
      }
    }
  }//anonymous scope end
...
model {
  ...
  wx_mu[1:3] ~ normal(0,0.02);
  wx_sigma[1:3] ~ normal(0,0.04);
  wx_z[,1] ~ std_normal();
  wx_z[,2] ~ std_normal();
  wx_z[,3] ~ std_normal();
  ...
  prob_sigma ~ std_normal();
  val_sigma ~ std_normal();
  
  val_rat_z ~ normal(val_pred, val_sigma);
...
generated quantities{
  ...
  for(i in 1:n_vrat){
    val_lik[i] = normal_lpdf(val_rat_z[i] | val_pred[i], val_sigma);
  }
  for(i in 1:n_prat){
