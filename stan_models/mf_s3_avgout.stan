...
transformed data{
  ...
  array[n_s] vector[n_t] cue_out_avg = rep_array(rep_vector(0,n_t),n_s); // Running average shown outcome for current cue (points scale); 0 if no prior shown outcomes
  ...
  for (s in 1:n_s) {
    array[n_f] real out_sum = rep_array(0.0,n_f); // Sum of shown outcomes for each cue in block
    array[n_f] int out_count = rep_array(0,n_f); // Number of shown outcomes for each cue in block
    ...
    for (t in 1:n_t) {
      ...
      if (t == 1 || t == (n_tb + 1)) {
        out_sum = rep_array(0.0,n_f);
        out_count = rep_array(0,n_f);
      }

      if (out_count[f] > 0) {
        cue_out_avg[s,t] = out_sum[f] / out_count[f];
      }

      // Keep this valence observation only if:
      // 1) valence was collected on this trial,
      // 2) outcome was shown on this trial.
      if (val_rat_num[s,t] != 0 && rew_hid[s,t] == 0) {
...
parameters {
  ...
  // Effect of average shown outcome for current cue (raw points scale)
  real w_pco_mu;
  ...
  // Effect of interaction: average shown cue outcome * current-trial outcome
  real w_int_mu;
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] w_pco = w_pco_mu + w_pco_sigma*w_pco_z; // Average shown outcome for current cue
    ...
    for (s in 1:n_s) {
      ...
      for (t in 1:n_t) {
        ...
        if (val_rat_num[s,t] != 0 && rew_hid[s,t] == 0) {
          real cue_avg_sc = cue_out_avg[s,t] / 200; // Scale to [-0.5, 0.5]
          real rew_sc = rew[s,t] / 200; // Scale to [-0.5, 0.5]
          real int_sc = cue_avg_sc * rew_sc; // Scale to [-0.25, 0.25]
