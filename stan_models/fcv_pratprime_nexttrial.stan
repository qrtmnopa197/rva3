...
// Also allows probability ratings to prime valence ratings on the next trial. Therefore, this model is identical
// to fcv_pratprime_ext.stan, but with the decay (kappa) fixed to 0.
...
parameters {
  ...
  // Effect of recent probability ratings (mean-centered) on valence
  real w_pr_mu;
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] kappa = rep_vector(0,n_s); // Fixed so only lag-1 priming remains
    ...
    vector[n_s] w_pr = w_pr_mu + w_pr_sigma*w_pr_z;
    ...
    row_vector[n_tb] decay_vec_pr; // Weights for past probability ratings
    row_vector[6] sum_preds; // Weighted sum of past predictors
    real sum_pr; // Weighted sum of past probability ratings
    
    for (s in 1:n_s) {
      decay_vec = pow(gamma[s],tb_vec); // Create the decay vector for this subject
      decay_vec_pr = pow(kappa[s],tb_vec); // Create the decay vector for this subject
      for (t in 1:n_t){
        if(t == 1){
...
model {
  ...
  // Similar prior on this as the effect of V - just rescaled 
  w_pr_mu ~ normal(0,6);
