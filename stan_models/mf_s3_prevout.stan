...
transformed data{
  ...
  array[n_s] vector[n_t] cue_out_prev = rep_array(rep_vector(0,n_t),n_s); // Most recent shown outcome for current cue (points scale)
  array[n_s,n_t] int cue_out_prev_avail = rep_array(0,n_s,n_t); // 1 if current cue has a prior shown outcome in this block
  ...
  for (s in 1:n_s) {
    array[n_f] real last_out = rep_array(0.0,n_f); // Most recent shown outcome for each cue in block
    array[n_f] int has_last_out = rep_array(0,n_f); // 1 if cue has at least one shown outcome in current block
    ...
    for (t in 1:n_t) {
      ...
      if (t == 1 || t == (n_tb + 1)) {
        last_out = rep_array(0.0,n_f);
        has_last_out = rep_array(0,n_f);
      }

      cue_out_prev_avail[s,t] = has_last_out[f];
      if (has_last_out[f] == 1) {
        cue_out_prev[s,t] = last_out[f];
      }

      // Keep this valence observation only if:
      // 1) valence was collected on this trial,
      // 2) outcome was shown on this trial, and
      // 3) this cue has a prior shown outcome in this block.
      if (val_rat_num[s,t] != 0 && rew_hid[s,t] == 0 && cue_out_prev_avail[s,t] == 1) {
        val_keep[val_rat_num[s,t]] = 1;
      }

      // Update cue-specific state only when current outcome is shown.
      if (rew_hid[s,t] == 0) {
        has_last_out[f] = 1;
        last_out[f] = rew[s,t];
      }
...
parameters {
  ...
  // Effect of previous shown outcome for current cue (raw points scale)
  real w_pco_mu;
  real<lower=0> w_pco_sigma;
  vector[n_s] w_pco_z;
  ...
  // Effect of interaction: previous shown cue outcome * current-trial outcome
  real w_int_mu;
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] w_pco = w_pco_mu + w_pco_sigma*w_pco_z; // Previous shown outcome for current cue
    vector[n_s] w_out = w_out_mu + w_out_sigma*w_out_z; // Current shown outcome effect
    vector[n_s] w_int = w_int_mu + w_int_sigma*w_int_z; // Outcome-by-outcome interaction
    ...
    for (s in 1:n_s) {
      ...
      real pr_prev = 0; // Previous-trial probability priming (not cue-specific)
      ...
      for (t in 1:n_t) {
        ...
        // Update global probability-rating priming state for trial t+1.
        // If no probability rating occurred on trial t, priming is set to 0.
        if (prob_rat_num[s,t] != 0) {
          pr_prev = prob_rat[prob_rat_num[s,t]] - 0.5;
        } else {
          pr_prev = 0;
        }
      }
