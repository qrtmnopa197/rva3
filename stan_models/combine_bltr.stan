// The preregistered model for RVA study 3, but with block and trial combined into one regressor.
...
data {
  ...
  array[n_s] vector[n_t] tr_sub_cent; // Subject-level trial number, mean-centered
}
...
parameters {
  ...
  real forget_cns_mu;
  real<lower=0> forget_cns_sigma;
  vector[n_s] forget_cns_z;
  
  // Forgetting rate when the cue is presented, but the reward is hidden (Reward Not Shown)
  real forget_rns_mu;
  real<lower=0> forget_rns_sigma;
  vector[n_s] forget_rns_z;
...
transformed parameters {
  ...
  {//anonymous scope start
    ...
    vector[n_s] forget_cns = inv_logit(forget_cns_mu + forget_cns_sigma*forget_cns_z); 
    vector[n_s] forget_rns = inv_logit(forget_rns_mu + forget_rns_sigma*forget_rns_z);   
    ...
    for (s in 1:n_s) {
      ...
      for (t in 1:n_t){
        ...
        V[s,t+1] = V[s,t]*forget_cns[s]; // Decay toward 0 by defafult
        if(rew_hid[s,t] == 0){
          ...
          // If the reward was hidden...
          V[s,t+1,frac[s,t]] = V[s,t,frac[s,t]]*forget_rns[s]; // Forget according to the rate for these trials
        ...
        if(val_rat_num[s,t] != 0){
          ...
          val_pred[val_rat_num[s,t]] = inv_logit(w_0[s] + w_tr[s]*tr_sub_cent[s,t]
...
model {
  ...
  forget_cns_mu ~ normal(-.05,1.7); //approx. equal to uniform distribution when passed through inv_logit
  forget_cns_sigma ~ normal(0,4);
  forget_cns_z ~ std_normal();
  
  forget_rns_mu ~ normal(-.05,1.7); //approx. equal to uniform distribution when passed through inv_logit
  forget_rns_sigma ~ normal(0,4);
  forget_rns_z ~ std_normal();
...
generated quantities{
  ...
  // Note the adjustment for the truncated distribution is taken from the Stan Reference Manual section 7.4
  for(i in 1:n_vrat){
