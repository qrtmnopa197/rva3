...
Run affect-only model with cue-linked probability predictor and reward-history term
```{r}
...
```{r}
mf_s3_eff <- get_draws("mf_s3",model_out_dir=model_out_dir,vars=c("w_prob_mu","w_int_mu"))

prob_eff_out100 <- mf_s3_eff[,,"w_prob_mu"] + 100*mf_s3_eff[,,"w_int_mu"]
prob_eff_outm100 <- mf_s3_eff[,,"w_prob_mu"] - 100*mf_s3_eff[,,"w_int_mu"]

cat("Effect of probability when outcome is +100\n")
get_ci_pd(draws=prob_eff_out100)

cat("\nEffect of probability when outcome is -100\n")
get_ci_pd(draws=prob_eff_outm100)
```
